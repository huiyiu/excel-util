...
public class ExcelReadHelper {
	...
	public static List<Object> excelRead(InputStream is, String[] properties, Class obj) throws Exception{
		Workbook book = null;
		try {
	...
	public static List<Object> excelRead(String filePath,String[] properties,Class obj) throws Exception{
		...
		return excelRead(new FileInputStream(file), properties, obj);
	}
		...
		if ("java.lang.String".equals(type) || "String".equals(type)) {
			...
			if (value.length() > 0){//当长度为19(yyyy-MM-dd HH24:mm:ss)或者为14(yyyyMMddHH24mmss)时Date格式转换为yyyyMMddHH24mmss
				if(value.length() == 19 || value.length() == 14){
					//oo[0] = new DateTime(value, "yyyyMMddHH24mmss");
					oo[0] = DateTime.parse(value,DateTimeFormat.forPattern("yyyyMMddHHmmss"));
				}
				else{     //其余全部转换为yyyyMMdd格式
					oo[0] = DateTime.parse(value,DateTimeFormat.forPattern("yyyyMMdd"));
